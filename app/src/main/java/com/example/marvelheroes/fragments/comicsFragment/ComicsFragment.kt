...
class ComicsFragment : Fragment() {
    ...
    private var adapter = ComicsAdapter()
    private val presenter = ComicsPresenter()
    private var comics: ComicDataWrapper? = null
    ...
    private var items = mutableListOf<ComicsAdapter.DataItem>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (comics == null) {
            if (!MyApplication.searchComics.isNullOrEmpty()) {
                previousSearch = MyApplication.searchComics
                restoreSearch = true
            }
    ...
    private fun getPresenterApiData(title: String? = null, offset: Int = 0) {
        ...
        if (comics == null) {
            val subscription = presenter.getComicsObject(title, offset)
                ?.observeOn(AndroidSchedulers.mainThread())
                ?.subscribeOn(Schedulers.io())
                ?.retry(5)
                ?.doOnSubscribe {
     ...
     */
    private fun setupUi(comics: ComicDataWrapper) {
        this.comics = comics
        if (comics?.code == 200) {
            if (comics.data?.count == 0) {
                if (offset > 0) {
                    Toast.makeText(
                        context,
                        R.string.no_more_comics_search,
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                ...
                preparingRVList(comics.data.results, offset)
                hideCustomProgressDialog()
            } else {
     ...
     */
    private fun preparingRVList(list: List<Comic>?, offsetAPI: Int) {

        if (offsetAPI == 0) {
            items =
                list?.map { ComicsAdapter.DataItem.ComicItem(it) }!!.toMutableList()

        } else {
            //items.removeLast()
            items.addAll(list?.map { ComicsAdapter.DataItem.ComicItem(it) }!!.toMutableList())
        }
